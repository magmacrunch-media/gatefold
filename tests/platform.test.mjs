...
function load(harness, ua) {
    const sandbox = harness.coreSandbox({
        navigator: { userAgent: ua, userAgentData: undefined },
        document: { querySelectorAll: () => [] },
    });
    harness.loadUI(sandbox, 'platform.js');
    return sandbox.Gatefold.platform;
}
...
export default function (M, harness) {
    test('a Mac is detected, and Windows is not', () => {
        ok(load(harness, MAC).isMac, 'macOS');
        ok(!load(harness, WIN).isMac, 'Windows');
    });
    ...
    test('userAgentData wins over the userAgent when it is there', () => {
        ...
        harness.loadUI(sandbox, 'platform.js');
        ok(sandbox.Gatefold.platform.isMac, 'the modern API is asked first');
    });

    test('on Windows every label is left exactly alone', () => {
        const P = load(harness, WIN);
        for (const s of ['Ctrl+S', 'Ctrl+Shift+S', 'Alt+F4', 'Del', 'Shift+arrows']) {
            eq(P.label(s), s, s);
        }
    });

    test('on a Mac the modifiers become glyphs', () => {
        const P = load(harness, MAC);
    ...
    test('Alt+F4 becomes the Mac way to quit', () => {
        eq(load(harness, MAC).label('Alt+F4'), '⌘Q', 'quit');
    });

    test('prose containing a chord is relabelled too', () => {
        const P = load(harness, MAC);
        eq(P.label('Undo (Ctrl+Z)'), 'Undo (⌘Z)', 'a tooltip');
        ok(P.label('FILE    Ctrl+N new · Ctrl+O open').includes('⌘N'),
            'the reference card');
    });
    ...
    test('a label with nothing to translate survives untouched', () => {
        ...
        eq(P.label('Layers'), 'Layers', 'plain words');
    });

    /* The markup carries the Windows spelling, which is what the relabeller
       expects to find. If a hint were written some other way it would simply
       not be translated, and the Mac build would show a Ctrl chord. */
    test('the markup spells its shortcuts the way the relabeller reads them', () => {
        ...
        for (const h of hints) {
            ok(/^(Ctrl\+|Alt\+|Shift\+|F\d|Del)/.test(h),
